...
class m190117_000003_fix_supertablecontent_tables extends Migration
{
    /**
     * @inheritdoc
     */
    public function safeUp()
    {
        ...
        foreach ($fields as $field) {
            ...
            if (isset($fieldsByHandle[$handle])) {
                ...
                $field['settings']['contentTable'] = '{{%stc_' . $handle . '_' . count($fieldsByHandle[$handle]) . '}}';
            } else {
                $field['settings']['contentTable'] = '{{%stc_' . $handle . '}}';
            }
        ...
        if (!empty($duplicateHandles)) {
            ...
            foreach (array_keys($duplicateHandles) as $handle) {
                ...
                if ($originalSchema === null) {
                    Craft::warning('Could not update the Super Table fields with the handle "' . $originalField['handle'] . '" because the content table doesn\'t exist.', __METHOD__);
                    continue;
                }
