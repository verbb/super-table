...
Craft.SuperTable.MatrixInputAlt = Garnish.Base.extend({
    id: null,
    blockTypes: null,
    ...
    init: function(id, blockTypes, inputNamePrefix, maxBlocks) {
        ...
        var $blocks = this.$blockContainer.children(),
            collapsedBlocks = Craft.SuperTable.MatrixInputAlt.getCollapsedBlockIds();
...
{
    collapsedBlockStorageKey: 'Craft-' + Craft.systemUid + '.SuperTable.MatrixInputAlt.collapsedBlocks',

    getCollapsedBlockIds: function() {
        if (typeof localStorage[Craft.SuperTable.MatrixInputAlt.collapsedBlockStorageKey] === 'string') {
            return Craft.filterArray(localStorage[Craft.SuperTable.MatrixInputAlt.collapsedBlockStorageKey].split(','));
        }
        else {
            return [];
        }
    ...
    setCollapsedBlockIds: function(ids) {
        localStorage[Craft.SuperTable.MatrixInputAlt.collapsedBlockStorageKey] = ids.join(',');
    },

    rememberCollapsedBlockId: function(id) {
        if (typeof Storage !== 'undefined') {
            var collapsedBlocks = Craft.SuperTable.MatrixInputAlt.getCollapsedBlockIds();

            if ($.inArray('' + id, collapsedBlocks) === -1) {
                collapsedBlocks.push(id);
                Craft.SuperTable.MatrixInputAlt.setCollapsedBlockIds(collapsedBlocks);
            }
        }
    },
...
var MatrixBlock = Garnish.Base.extend({
    ...
    collapse: function(animate) {
        ...
        // Remember that?
        if (!this.isNew) {
            Craft.SuperTable.MatrixInputAlt.rememberCollapsedBlockId(this.id);
        }
        else {
    ...
    expand: function() {
        ...
        if (!this.isNew) {
            Craft.SuperTable.MatrixInputAlt.forgetCollapsedBlockId(this.id);
        }
